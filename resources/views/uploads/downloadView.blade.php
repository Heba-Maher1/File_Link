<x-main-layout title="Download">
    <div class="mt-5 shadow-lg" style="width: 400px; background: white; padding: 30px; border-radius: 10px;">
        ...
        <div class="d-flex justify-content-between align-items-center border-bottom mb-4">
            <p class="text-secondary ">{{ $file->name }} <br> <span style="font-size: 15px">{{ $file->size }}
                    KB</span></p>
        ...
        <a href="{{ route('download', $file->shared_link) }}" class="btn btn-lg btn-block w-100 text-white mb-2"
            style="background: #456991">Download</a>

        <a class="link-danger link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover"
            href="{{ route('index') }}">Send another?</a>
    </div>

    <div id="detailContainer" class=" position-absolute bg-white d-none"
        style="height: 100vh;width:50%;top:0;right:0;z-index:4;">
        <div class="nav border-bottom p-3">
            <i id="close" class="fa-regular fa-circle-xmark"
                style="color:gainsboro;font-size:30px;cursor: pointer;"></i>
        </div>
            ...
            <table>
                ...
                @foreach ($files as $fileItem)
                    <tr class="border-bottom py-3">
                        ...
                        <td>
                            <form action="{{ route('destroy', $fileItem->id) }}" method="POST">
                                @method('delete')
                                @csrf
                                <button type="submit" class="btn border-0"><i
